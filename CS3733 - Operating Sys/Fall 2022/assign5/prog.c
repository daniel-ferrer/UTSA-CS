...
{ 
    ...
    int PID, PR, numCPUBurst, numIOBurst, CPUindex, IOindex;

    time_t start, timeEnterReadyQ;

    struct PCB *pPrev, *pNext;
    struct timespec ts_begin, ts_end, ts_beginUtil, ts_endUtil, beginTime, endTime;    
}; 

int ALG;
char *inputFile;

struct PCB *Ready_Q, *pHead_IO;
struct PCB *IO_Q = NULL, *pHead = NULL;

pthread_mutex_t lock;

sem_t sem_CPU, sem_IO;

struct timespec ts, ts_begin, ts_end;

int process = 0, quantumTime = 0, totalProcesses = 0, TotalSleepTime = 0;
int file_read_done = 0, cpu_sch_done = 0, io_sys_done = 0, cpu_busy = 0, io_busy = 0;

double totalCPU, totalThroughput, CPU_Util, totalWait, waitTime, turnAroundTime, throughput, totalTime;
...
{ 
    ...
    { 
        ...
        int i = 0, count = 0;
        int PR, CPUBurstCount, IOBurstCount, sleepTime; 
...
{
    int i, inputCNT = 0, algoARGS = 0;
    pthread_t tid1, tid2, tid3;

    if(argc < 4) 
    { 
        fprintf(stderr, "ERROR args, Usage: prog -alg [FIFO|SJF|PR|RR] [-quantum integer(ms)] -input [input_file_name.txt]\n"); 
        exit(-1);
    } 
    ...
    { 
        ...
        { 
            fprintf(stderr, "ERROR prog, Usage: prog -alg [FIFO|SJF|PR|RR] [-quantum integer(ms)] -input [input_file_name.txt]\n"); 
            exit(-1);
        } 
        if(strcmp(argv[1], "-alg") != 0) 
        { 
            fprintf(stderr, "ERROR alg, Usage: prog -alg [FIFO|SJF|PR|RR] [-quantum integer(ms)] -input [input_file_name.txt]\n"); 
            exit(-1);
        } 
